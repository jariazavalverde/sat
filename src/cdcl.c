...
CDCL_Graph *cdcl_graph_alloc(int nbvar) {
	int i;
	CDCL_Graph *G = malloc(sizeof(CDCL_Graph));
	G->nodes = malloc((nbvar+1) * sizeof(CDCL_Node));
	G->size = nbvar;
	G->decision_level = 0;
	for(i = 0; i <= nbvar; i++)
		G->nodes[i] = NULL;
...
void cdcl_graph_set_node(CDCL_Graph *G, Atom atom, Bool value, int level, Decision decision, Clause *clause) {
	...
	CDCL_Node *node = malloc(sizeof(CDCL_Node));
	G->nodes[index] = node;
	node->atom = atom;
	node->value = value;
		...
		if(!F->sat_clauses[clause->id]) {
			...
			// else, remove the literal from the clause
				...
				if(clause->length == 1) {
					...
					cdcl_graph_set_node(G, atom, value, G->decision_level, CONFLICTIVE, clause);
					return 0;
				}
...
int cdcl_decision_state(Formula *F, CDCL_Graph *G, Trace *trace) {
	...
	if(clause_node != NULL) {
		...
		if(literal_node != NULL) {
			...
			cdcl_graph_set_node(G, atom, value, G->decision_level, ARBITRARY, NULL);
			F->nb_decisions++;
	...
	while(F->lst_unit_clauses != NULL) {
		...
		for(i = 0; i < length; i++) {
			...
			if(F->sat_clauses[clause->id] == 0 && clause->length == 1 && F->interpretation[atom] == UNKNOWN) {
				...
				trace_push(trace, NULL, atom, NONE);
				// Set graph node
				cdcl_graph_set_node(G, atom, value, G->decision_level, FORCED, clause);
				// Replace variable
